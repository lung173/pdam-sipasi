    ...
    <div className="space-y-6">
      ...
      <div className="card">
        <div className="px-5 py-4 border-b border-gray-100 dark:border-slate-800 flex items-center justify-between">
          <h2 className="font-semibold text-gray-900 dark:text-white">Antrian Dokumen — Perlu Keputusan</h2>
          <Link href="/dashboard/direktur/antrian" className="text-sm text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1">
        ...
        {antrianDocs.length === 0 ? (
          ...
          <div className="divide-y divide-gray-100 dark:divide-slate-800">
            {antrianDocs.map((doc) => (
              <div key={doc.id} className="flex items-center gap-4 px-5 py-4 hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                <div className="flex-1 min-w-0">
                  <p className="font-mono text-xs text-blue-700 dark:text-blue-400 mb-0.5">{doc.nomorSurat}</p>
                  <p className="text-sm font-semibold text-gray-900 dark:text-white truncate">{doc.perihal}</p>
      ...
      {recentDecisions.length > 0 && (
        <div className="card">
          <div className="px-5 py-4 border-b border-gray-100 dark:border-slate-800">
            <h2 className="font-semibold text-gray-900 dark:text-white">Riwayat Keputusan Terbaru</h2>
          </div>
          <div className="divide-y divide-gray-100 dark:divide-slate-800">
            {recentDecisions.map((d) => (
              <div key={d.id} className="flex items-center gap-4 px-5 py-3">
                <div className="flex-1 min-w-0">
                  <p className="font-mono text-xs text-gray-500 dark:text-slate-500">{d.suratMasuk.nomorSurat}</p>
                  <p className="text-sm font-medium text-gray-800 dark:text-slate-200 truncate">{d.suratMasuk.perihal}</p>
                </div>
                <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${
                  d.decisionType === "DISETUJUI" ? "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400" :
                  d.decisionType === "DITOLAK"   ? "bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400" :
