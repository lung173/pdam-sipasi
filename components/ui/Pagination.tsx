  ...
  return (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-4 border-t border-gray-100 dark:border-slate-800">
      <p className="text-sm text-gray-500 dark:text-slate-400">
        Menampilkan <span className="font-medium text-gray-700 dark:text-slate-300">{from}–{to}</span> dari{" "}
        <span className="font-medium text-gray-700 dark:text-slate-300">{totalItems}</span> dokumen
      </p>
      ...
      <div className="flex items-center gap-1">
        ...
        {pages.map((p, i) =>
          p === "..." ? (
            <span key={`ellipsis-${i}`} className="px-2 text-gray-400 dark:text-slate-500 text-sm select-none">
              …
            </span>
          ) : (
            <button
